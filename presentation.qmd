...
## Geospatial approximation of molecular epidemiological methods

::: {style="display: flex; justify-content: center; margin-top: 20px; margin-bottom: 30px;"}
::: {style="background-color: #f8f9fa; border: 2px solid #3d763d; border-radius: 15px; padding: 25px; text-align: center; max-width: 85%; box-shadow: 0 4px 6px rgba(0,0,0,0.1);"}
**Increased precision and mechanistic insight in spatial epidemiology**
:::
:::
...
::: {style="background-color: #f8f9fa; border: 2px solid #3d763d; border-radius: 15px; padding: 25px; text-align: center; max-width: 85%; box-shadow: 0 4px 6px rgba(0,0,0,0.1);"}
**Relates External Exposure to Target Organ Dose and Observed Outcomes**
:::
:::
...
3) Documented and Open-Source tools and pipelines are needed to ensure projects can build upon each other
...
#| code-line-numbers: "|3-4|13|22"

# Download and process maximum temperature (tmmx)
download_data(
...
# Create H3 hexagonal grid and extract tmmx values
...
amadeus ──► [Download: EPA AQS, MODIS, MERRA-2, NARR, NLCD, HMS ...]
           ...
           [Covariate calculation at AQS sites & prediction grid]
                  │
            [Base Learners: LightGBM | Neural Net | glmnet | GAM]
                  │
...
**Meta learner:**
- Out-of-Sample Learning Only (Prevents Meta-Learning Data Leakage)
- Another ML Model: LightGBM or GAM

**Validation:**

- Space-Time cross-validation (`{spatialsample}`)
...
## Beethoven Extensions for Exposomic Modeling 

::: {style="display: flex; justify-content: center; margin-top: 30px;"}
::: {style="background-color: #f8f9fa; border: 2px solid #3d763d; border-radius: 15px; padding: 25px; text-align: center; max-width: 85%; box-shadow: 0 4px 6px rgba(0,0,0,0.1);"}
**The Extensibility of Beethoven Serves as a Template for Predictive Modeling of Exposomic Chemicals**

1. Wildfire PM2.5 Model 
2. PM2.5 Chemical Speciation Model 
3. Hazardous Air Pollutants 
4. Groundwater Pesticides
...
::: {style="display: flex; justify-content: center; margin-top: 30px;"}
::: {style="background-color: #f8f9fa; border: 2px solid #3d763d; border-radius: 15px; padding: 25px; text-align: center; max-width: 85%; box-shadow: 0 4px 6px rgba(0,0,0,0.1);"}
**SET group focuses on updatable, reproducible models and chemicals with sparse exposure information for risk assessment**
:::
:::
...
- **Chemical concentrations** — spatiotemporal exposure estimates
- **Toxicokinetic parameters** — HTTK model outputs (age, sex, body weight distributions)
- **In vitro concentration-response** — Tox21 / InvitroDB assay data
...
![](images/GeoTox-01.png){fig-align="center" width="60%"}

**Initialize GeoTox analysis**

- Load required packages (`GeoTox`, `sf`, `dplyr`)
- Define study region, areeal units, time period
- Fit inidivual chemical-assay dose-response models 

## 2. Tidy-style workflow 

![](images/GeoTox-02.png){fig-align="center" width="60%"}

**Spatially-referenced chemical concentrations**

- Simulate stead-state plasma concentrations 
- Simulate population characteristics 
- Mixtures dose-response modeling 
- Perform sensitivity analyses

## 3. Object-Oriented Database Structure

![](images/GeoTox-03.png){fig-align="center" width="60%"}

**Age and sex distributions by location**

- GeoTox object
- DuckDB relational objects for datasets and analysis results

## 4. Plot Normalized External Exposures

![](images/GeoTox-04.png){fig-align="center" width="60%"}


## 5. Plot Internal and In-vitro Equivalent Concentrations

![](images/GeoTox-06.png){fig-align="center" width="60%"}

**Geo-referenced internal dose and in-vitro equivalent concentrations**

- In-vitro equivalent concentrations puts units on the same scale as in-vitro assays
- Common molecular initiating event (MIE) or Key Event 
- Mechanistic assays are now linked to geospatial exposure and population data

## 7. Plot GeoTox Risk Predictions

![](images/GeoTox-07.png){fig-align="center" width="60%"}

 - Concentration Addition 
 - Independent Action
 - Hazard Index

![](images/GeoTox-08.png){fig-align="center" width="60%"}

**Spatial distribution of risk**

- Areal unit plots of assay based risk
- Identify vulnerable populations and hotspots

## 8. Sensitivity Analysis

![](images/GeoTox-09.png){fig-align="center" width="60%"}
...
# Summary {.section-title background-color="#3d763d"}

<!-------------------------------------------------------------------------->
<!--------------------------------------------------------------------------->
## Source-to-Outcome: Current Developments

::: {.incremental}
- Geospatially mapping population risk based on perturbation of mechanistic endpoints
- Geospatially mapping individual risk based on mechanistic endpoints (Paper in progress, Reif et al.)
- High temporal resolution dose-response and risk assessment based on GeoTox (Paper in progress, Kruse et al.)
- Geospatial mapping of risk based on multiple assay endpoints (Messier, Marvel, Reif, 2025)
:::
...
## Source-to-Outcome: Future Directions

::: {.incremental}
- GeoTox providing a framework for near-real time risk assessment during disaster events (e.g. East Palestine train derailment)
- Combining multiple mechanistic pathways to predict complex health outcomes 
- Limitation: Address the validation issue — how do we validate mechanistic risk predictions when we don't have observed data available?
- Limitation: Address the uncertainty issue — how do we quantify and communicate the uncertainty in mechanistic risk predictions?
- Limitation: Geospatial exposures and a lamp-post problem - keep expanding the availability of geospatial exposure data to cover more chemicals and exposure routes
...
- Cavin Ward-Caviness (Former EPA, Buck Institute)
